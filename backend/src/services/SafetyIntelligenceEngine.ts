...
export class SafetyIntelligenceEngine {
   ...
   */
  static async calculateSafetyScore(childId: string): Promise<{ score: number; level: string; factors: string[] }> {
    try {
      const now = new Date();
      const last7Days = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);

      const [safetyEvents, websiteBlocks, deviceHealth] = await Promise.all([
        SafetyEvent.find({ childId, createdAt: { $gte: last7Days } }),
        WebsiteRule.find({ childId, isBlocked: true }), // Simulated blocks count could come from a history table
        DeviceHealth.findOne({ childId }).sort({ lastSeen: -1 }),
      ]);
   ...
   */
  static async getSafetyTrends(childId: string, days: number = 7) {
    try {
      const now = new Date();
      ...
      const events = await SafetyEvent.aggregate([
        { $match: { childId: childId as any, createdAt: { $gte: startDate } } },
        {
          $group: {
