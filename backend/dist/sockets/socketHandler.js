...
exports.setupSockets = exports.getIo = void 0;
...
const setupSockets = (io) => {
    ...
    io.on('connection', (socket) => {
        ...
        if (socket.user?.role === 'parent') {
            ...
            updateDeviceStatus(socket.user.deviceId, true).then(device => {
                if (device) {
                    ...
                    Child_1.Child.findById(device.childId).then(child => {
                        if (child) {
        ...
        socket.on('parent:command', async (data, callback) => {
            ...
            try {
                ...
                if (Date.now() - lastCmd < COMMAND_LIMIT_MS) {
                    if (callback)
                        callback({ success: false, error: 'Rate limit exceeded. Please wait.' });
                    return;
                }
                ...
                const child = await Child_1.Child.findOne({ _id: childId, parentId: socket.user.parentId });
                if (!child)
                    throw new Error('Unauthorized child');
                const device = await Device_1.Device.findOne({ _id: deviceId, childId: child._id });
                if (!device)
                    throw new Error('Unauthorized device');
                // Log command
                const command = await CommandAudit_1.CommandAudit.create({
                ...
                if (callback)
                    callback({ success: true, commandId: command.commandId });
            }
            catch (err) {
                if (callback)
        ...
        socket.on('device:usage:updated', async (data) => {
            if (socket.user?.role === 'device') {
                const device = await Device_1.Device.findById(socket.user.deviceId);
                if (device) {
                    const child = await Child_1.Child.findById(device.childId);
                    if (child) {
...
async function updateDeviceStatus(deviceId, isOnline, batteryLevel) {
    try {
        const update = { isOnline, lastSeen: new Date() };
        if (batteryLevel !== undefined)
            update.batteryLevel = batteryLevel;
        return await Device_1.Device.findByIdAndUpdate(deviceId, update, { new: true });
    }
    catch (error) {
