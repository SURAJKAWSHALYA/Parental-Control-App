...
exports.deleteMessage = exports.markAsRead = exports.sendMessage = exports.getMessages = exports.createConversation = exports.getConversations = void 0;
...
const verifyFamilyOwnership = async (user, conversationId) => {
    const conversation = await Conversation_1.Conversation.findById(conversationId);
    if (!conversation)
        return null;
    if (user.role === 'parent' && conversation.parentId.toString() !== user.id)
        return null;
    if (user.role === 'child' && conversation.childId.toString() !== user.id)
        return null;
    return conversation;
};
...
const createConversation = async (req, res) => {
    try {
        ...
        if (!child)
            return res.status(404).json({ success: false, message: 'Child not found in family' });
        let conversation = await Conversation_1.Conversation.findOne({ parentId, childId, type: 'DIRECT' });
        if (!conversation) {
            conversation = await Conversation_1.Conversation.create({ parentId, childId, type: 'DIRECT' });
        }
...
const getMessages = async (req, res) => {
    try {
        ...
        let messages = await Message_1.Message.find(query)
...
const sendMessage = async (req, res) => {
    try {
        ...
        // Dedup check
        if (clientMessageId) {
            const existing = await Message_1.Message.findOne({ clientMessageId, conversationId });
            if (existing)
                return res.json({ success: true, data: existing, duplicated: true });
        }
        ...
        safetyWorker.enqueueMessageAnalysis(message._id, conversation.parentId, conversation.childId, text);
...
const deleteMessage = async (req, res) => {
    try {
        ...
        await Message_1.Message.findByIdAndDelete(id);
        const io = (0, socketHandler_1.getIo)();
        const targetRoom = req.user.role === 'parent' ? `device_${conversation.childId}` : `parent_${conversation.parentId}`;
