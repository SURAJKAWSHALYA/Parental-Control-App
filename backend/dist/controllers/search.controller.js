...
const globalSearch = async (req, res) => {
    try {
        ...
        const results = [];
        // 1. Search Children
        const children = await Child_1.Child.find({ parentId: pId, name: regex });
        ...
        // 2. Search Devices
        const devices = await Device_1.Device.find({ childId: { $in: childIds }, $or: [{ deviceName: regex }, { deviceModel: regex }] });
